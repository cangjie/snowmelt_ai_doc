"""一次性产物：万龙服务中心零售财年报表按「七色米订单号」合并展示商品明细。
...
输出形态（与南山一致）：
    [1] 另存独立备份 wanlong_service_retail_orders_fy_with_detail.xlsx（单 sheet）
...
用法（确保产物未被 Excel 打开；源报表被打开不影响，只读加载）：
    python3 add_wanlong_service_retail_detail_merged_xlsx.py
"""
import os
import sys
...
SRC_XLSX = os.path.join(
    BASE, "wanlong_service_retail_orders_fy_2025-05-01_2026-04-30.xlsx")
OUT_XLSX = os.path.join(
    BASE, "wanlong_service_retail_orders_fy_with_detail.xlsx")
...
# 用户指定剔除：两笔 ¥0.02 测试单（类似养护 WF_YH_251110_00017 的测试双插）
EXCLUDE_CODES = {"WF_LS_251013_00001", "WF_LS_251013_00002"}
CODE_HEADER = "订单号"
