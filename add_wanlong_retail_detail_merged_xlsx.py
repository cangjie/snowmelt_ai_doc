"""一次性产物：万龙体验中心零售财年报表按「七色米订单号」合并展示商品明细。

规则完全沿用南山版 add_retail_detail_merged_xlsx.py，仅换输入/输出文件。

匹配键：
    年度零售「七色米订单号」 == 销售明细单1「单据编号」（均为 XSDxxxx 格式）

万龙特殊说明（与南山的唯一口径差异）：
    万龙有两个零售报表（万龙体验中心 / 万龙服务中心），但两店的销售明细
    都在同一个 万龙_销售单列表.xls 里。因此本脚本只做「报表订单 → 明细」
    的正向匹配；**不做**「明细记录是否存在于本报表」的反向核对
    （某条明细不在万龙服务中心，可能属于万龙体验中心，属正常）。
    红色 FF9999 仍保留：表示「本报表内非关闭单未匹配到明细」的正向异常。

输出形态（与南山一致）：
    [1] 另存独立备份 wanlong_retail_orders_fy_with_detail.xlsx（单 sheet）
...
用法（确保产物未被 Excel 打开；源报表被打开不影响，只读加载）：
    python3 add_wanlong_retail_detail_merged_xlsx.py
"""
import os
import sys
...
SRC_XLS = os.path.join(BASE, "万龙_销售单列表.xls")
SRC_XLSX = os.path.join(
    BASE, "wanlong_retail_orders_fy_2025-05-01_2026-04-30.xlsx")
OUT_XLSX = os.path.join(
    BASE, "wanlong_retail_orders_fy_with_detail.xlsx")
...
# 用户指定剔除：9 笔微额 ¥0.0x + 5 笔 ¥0 测试单（均无七色米号，无明细）。
# 3 笔实额（WT_LS_251109_00006 ¥360 / 251201_00003 ¥5979 / 260207_00002 ¥1500）
# 不剔除，保持标红（真实订单缺七色米引用）。
EXCLUDE_CODES = {
    "WT_LS_251011_00004", "WT_LS_251012_00002", "WT_LS_251012_00007",
    "WT_LS_251012_00008", "WT_LS_251012_00011", "WT_LS_251012_00014",
    "WT_LS_251012_00016", "WT_LS_251013_00001", "WT_LS_260111_00001",
    "WT_LS_251025_00005", "WT_LS_251028_00004", "WT_LS_251028_00005",
    "WT_LS_251106_00001", "WT_LS_260125_00002",
}
CODE_HEADER = "订单号"
...
def main():
    ...
    print(f"销售明细单1: {n_docs} 单据 / {n_det_rows} 明细行（含万龙两店）")
